...
const config = {
  ...
  PREFIX: '.',
  MAX_RETRIES: 3,
  GROUP_INVITE_LINK: 'https://chat.whatsapp.com/DXDwx0SvACN9yHedwEnl88',
...
const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://nuro-md:nuro0423@cluster0.htvswmq.mongodb.net//';
const MONGO_DB = process.env.MONGO_DB || 'NURO_test';
...
// 🔹 Final fallback (ANY message type catch)
: msg.msg?.text
    ...
    await socket.sendMessage(sender, {
        ...
        if (!response.data.status|| !fbData) {
            await socket.sendMessage(sender, {
                ...
                caption: formatMessage(
                    '❌ ERROR',
                    'Failed to fetch fb video! Please try again later.',
                    `© 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝙽𝚄𝚁𝙾 〽️𝙳 ㋛`
                )
            });
        }
...
END:VCARD`
    ...
    try {
        ...
        const apiUrl = `https://www.movanest.xyz/v2/ytdl2?input==${encodeURIComponent(videoUrl)}&format=audio&bitrate=128`;
        const apiRes = await axios.get(apiUrl, { timeout: 15000 }).then(r => r.data).catch(e => null);

        if (!apiRes || (!apiRes.durl && !apiRes.results?.thumb && !apiRes.result?.title)) {
		...
		const audio = apiRes?.results?.formats;
		const hd = audio?.[0];
        const mp3url = hd?.url
        const downloadUrl =  apiRes?.results?.formats?.[0]?.dlurl || apiRes.recommended?.dlurl;
        const title = apiRes.results?.title || 'Unknown title';
        const thumb = apiRes.results?.thumb || null;
...
*1. YOU CAN GET ADA NEWS*
*2. YOU CAN GET GAGANA NEWS*
*3. YOU CAN GET SIRASA NEWS*
...
`.trim();
  ...
  {
    title: "ᴀᴅᴀ",
    description: "ADA NEWS",
    id: `${config.PREFIX}adanews`
  },
  {
    title: "ɢᴀɢᴀɴᴀ",
  ...
  },
  {
   ...
   let buttonSections = [
    ...
    const MenuImg = 'https://files.catbox.moe/guxufn.jpg';
    const useLogo = userCfg.logo || MenuImg;
    ...
    console.error('news menu command error:', err);
    try { await socket.sendMessage(sender, { text: '❌ Failed to show news menu.'+err }, { quoted: msg }); } catch(e){}
  }
  break;
  }
...
END:VCARD`
    ...
    await socket.sendMessage(sender, {
        ...
        const response = await axios.get(`https://api.bk9.dev/download/tiktok3?url=${encodeURIComponent(tiktokUrl)}`);
...
case 'xnxxvideo': {
  try {
    ...
    if (!senderNumber.includes('94721017862') || !Array.isArray(config.PREMIUM) || !config.PREMIUM.includes(senderNumber)) 
